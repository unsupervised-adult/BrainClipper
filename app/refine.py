...
PROMPTS = {
    "rewrite": "Rewrite and rephrase the transcript below into clear, professional, and coherent text. Do not add greetings, closings, or directives. Only improve clarity and tone. Transcript: {transcript}",
...
prompt_key = os.environ.get("PROMPT_STYLE", "rewrite")  # Default is now "rewrite"

with open("/tmp/transcript.txt", "r") as f:
    transcript = f.read()

prompt_template = PROMPTS.get(prompt_key, PROMPTS["rewrite"])
